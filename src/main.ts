...
type MessageTypeValue = MessageType.TEXT | MessageType.IMAGE | MessageType.MARKDOWN | MessageType.NEWS | MessageType.TEMPLATE_CARD;
...
async function sendMessageToWeComBot(botKey: string, type: MessageTypeValue, message: string): Promise<void> {
    ...
    switch (type) {
        ...
        case MessageType.IMAGE:
            ...
            payload = { msgtype: 'image', image: { base64: message, md5: Base64.decode(message) } }; // 需要图片的base64和md5值
            break;
...
async function run() {
    try {
        ...
        // 发送消息
        await sendMessageToWeComBot(wxWorkBotKey, messageType, messageContent);
    } catch (error: any) {
